...
class Grid {
    ...
    static cloneMatrix(matrix) {
        return Array.from({ length: matrix.length }, (_, row) => Array.from({ length: matrix[0].length }, (_, col) => matrix[row][col]));
    }
    ...
    static fromMatrix(matrix) {
        const grid = Grid.fromProportions(matrix.length, matrix[0].length, null);
        grid.forEach((_, r, c) => {
    ...
    static generateMatrix(rows, cols, defaultVal) {
        return Array.from({ length: rows }, () => Array.from({ length: cols }, () => typeof defaultVal === 'function'
            ? defaultVal()
            : defaultVal));
    }
    ...
    rotateClockwise() {
        ...
        return Grid.fromMatrix(matrix[0].map((val, index) => matrix.map(row => row[index]).reverse()));
    }
